...
-- // 2. RAYFIELD KÜTÜPHANESİ KURULUMU \\ --
...
local Window = Rayfield:CreateWindow({
   ...
   KeySystem = false -- Key ve Webhook kaldırıldı
})
...
TabAI:CreateToggle({
   Name = "ACTIVATE L.A.I.S v3.0 NEURAL",
   CurrentValue = false,
   Flag = "AI_Master", 
   Callback = function(Value)
       AI_State.Enabled = Value
       if Value then
           notify("L.A.I.S v3.0", "Neural Engine Online. Smart boost features activated.")
...
TabVisuals:CreateToggle({
   Name = "Interface GUI Optimizer (Hide Others)",
   CurrentValue = false,
   Callback = function(Value)
...
TabInfo:CreateParagraph({Title = "Change log", Content = "Back to stable version,minimal UI update,gui opt., ai culling(BETA,network stabilizer v2,hardware accel."})
